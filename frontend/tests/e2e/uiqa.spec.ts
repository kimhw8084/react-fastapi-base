...
import {visualizationDisplayLabel} from '../../src/platform/workspace/visualizationDisplay'
...
async function revealResponsiveWorkspaceOptions(page:Page):Promise<void>{
 ...
 if(!open){await disclosure.locator('summary').click();await expect(disclosure).toHaveAttribute('open','')}
 await expect(selector).toBeVisible({timeout:10000})
}

async function displayPreferenceControl(page:Page,label:string):Promise<Locator>{
 const mobileTrigger=page.locator('.mobile-display-trigger')
 ...
 if(await mobileTrigger.isVisible()){
  ...
  const control=dialog.locator(`#mobile-${key}`);await expect(control).toHaveAccessibleName(label);return control
 }
 const control=page.locator(`.desktop-display-controls #desktop-${key}`);await expect(control).toHaveAccessibleName(label);return control
}
...
async function focusEvidence(page:Page,owner:Locator,control:Locator){
 ...
 const viewportTolerance=1/viewport.devicePixelRatio;return {...controlEvidence,owner_bounds:{left:ownerBox.x,right:ownerBox.x+ownerBox.width,top:ownerBox.y,bottom:ownerBox.y+ownerBox.height},viewport_bounds:{left:0,right:viewport.width,top:0,bottom:viewport.height},viewport_tolerance_device_px:1,fully_inside_owner:focusBounds.left>=ownerBox.x&&focusBounds.right<=ownerBox.x+ownerBox.width&&focusBounds.top>=ownerBox.y&&focusBounds.bottom<=ownerBox.y+ownerBox.height,fully_inside_viewport:focusBounds.left>=-viewportTolerance&&focusBounds.right<=viewport.width+viewportTolerance&&focusBounds.top>=-viewportTolerance&&focusBounds.bottom<=viewport.height+viewportTolerance}
}
...
async function expectRenderedMode(page:Page,mode:string):Promise<void>{
 const projection=projectionLocator(page,mode);if(await projection.count())await expect(projection).toBeVisible();else{const summary=page.locator('.workspace-summary').getByText(visualizationDisplayLabel(mode),{exact:true});if(await summary.count())await expect(summary).toBeVisible();else await expect(page.getByRole('heading',{name:'No matching records',exact:true})).toBeVisible()}
}
...
matrixTest('chg153-system-responsive-geometry',async({page,proof})=>{
 ...
 for(const width of [1440,390,320]){
  ...
  expect(controls.tablistCount).toBe(1);expect(controls.tabCount).toBe(9);expect(controls.visibleTabCount).toBe(9);expect(controls.selected).toEqual(['Notifications']);expect(keyboardClickCount.count).toBe(1);expect(keyboardClickCount.focusVisible).toBe(true);for(const focus of [firstFocus,middleFocus,finalFocus]){expect(focus.active,JSON.stringify(focus)).toBe(true);expect(focus.focusVisible,JSON.stringify(focus)).toBe(true);expect(focus.outlineStyle,JSON.stringify(focus)).not.toBe('none');expect(Number.parseFloat(focus.outlineWidth),JSON.stringify(focus)).toBeGreaterThan(0);expect(Number.parseFloat(focus.outlineOffset),JSON.stringify(focus)).toBeGreaterThan(0);expect(focus.fully_inside_owner,JSON.stringify(focus)).toBe(true);expect(focus.fully_inside_viewport,JSON.stringify(focus)).toBe(true)}
...
matrixTest('chg153-visualization-responsive-geometry',async({page,proof})=>{
 ...
 const captureConsumer=async(route:string,name:string,width:number,height:number)=>{
  await page.setViewportSize({width,height});await page.goto(route);await page.evaluate(()=>localStorage.clear());await page.reload();try{await revealResponsiveWorkspaceOptions(page)}catch(error){throw new Error(`Visualization selector did not load for ${route} at ${width}px: ${String(error)}`)}const owner=page.getByRole('group',{name:'Visualization'}),buttons=owner.getByRole('button'),count=await buttons.count();expect(count).toBeGreaterThanOrEqual(3);const modes=await buttons.evaluateAll(elements=>elements.map(element=>element.getAttribute('data-visualization-key')??'')),modeLabels=await buttons.allTextContents(),first=buttons.first(),middle=buttons.nth(Math.floor(count/2)),last=buttons.last();const middleIndex=Math.floor(count/2);const before=await pageGeometry(page);const ownerBefore=await owner.evaluate(element=>{const bounds=element.getBoundingClientRect(),style=getComputedStyle(element);return {clientWidth:element.clientWidth,scrollWidth:element.scrollWidth,left:bounds.left,right:bounds.right,overflowX:style.overflowX}})
  await first.click();await expect(first).toHaveAttribute('aria-pressed','true');await expectRenderedMode(page,modes[0]!.toLowerCase());await revealResponsiveWorkspaceOptions(page);await pointerScrollToEnd(page,owner,last);const lastVisibleByPointer=await boxFullyInside(owner,last);await expect(last).toBeVisible();await last.click();await revealResponsiveWorkspaceOptions(page);await expect(last).toHaveAttribute('aria-pressed','true');await expect(page).toHaveURL(new RegExp(`[?&]visualization=${encodeURIComponent(modes.at(-1)!.toLowerCase())}(?:&|$)`));await expectRenderedMode(page,modes.at(-1)!.toLowerCase());pointerWorked=pointerWorked||lastVisibleByPointer
  await page.goto(route);await page.evaluate(()=>localStorage.clear());await page.reload();await revealResponsiveWorkspaceOptions(page);const keyboardOwner=page.getByRole('group',{name:'Visualization'}),keyboardButtons=keyboardOwner.getByRole('button'),keyboardFirst=keyboardButtons.first(),keyboardMiddle=keyboardButtons.nth(middleIndex),keyboardLast=keyboardButtons.last();await keyboardFirst.focus();await page.keyboard.press('Shift+Tab');await page.keyboard.press('Tab');await expect(keyboardFirst).toBeFocused();await page.waitForTimeout(20);const firstFocus=await focusEvidence(page,keyboardOwner,keyboardFirst);for(let index=0;index<middleIndex;index+=1)await page.keyboard.press('Tab');await expect(keyboardMiddle).toBeFocused();await page.waitForTimeout(20);const middleFocus=await focusEvidence(page,keyboardOwner,keyboardMiddle)
 ...
 await page.setViewportSize({width:390,height:844});await page.goto('/plan-tasks');await page.evaluate(()=>localStorage.clear());await page.reload();await revealResponsiveWorkspaceOptions(page);const planningOwner=page.getByRole('group',{name:'Visualization'}),gantt=planningOwner.getByRole('button',{name:'Gantt',exact:true}),search=page.getByRole('textbox').first();await search.waitFor({state:'visible'});await gantt.click();await search.fill('task');await page.waitForTimeout(350);const filter=page.locator('.command-bar select').first();let filterValue:string|null=null;if(await filter.count()&&await filter.locator('option').count()>1){await filter.selectOption({index:1});filterValue=await filter.inputValue()}await page.setViewportSize({width:1440,height:900});await page.waitForTimeout(100);await page.setViewportSize({width:390,height:844});await expect(page.locator('.workspace-controls-disclosure > summary')).toContainText('Display: Gantt');await revealResponsiveWorkspaceOptions(page);await expect(gantt).toHaveAttribute('aria-pressed','true');await expect(search).toHaveValue('task');if(filterValue!==null)await expect(filter).toHaveValue(filterValue);const preserved={mode:await gantt.getAttribute('aria-pressed'),search:await search.inputValue(),filter:filterValue,url:page.url(),summary:await page.locator('.workspace-summary').getByText('Gantt',{exact:true}).isVisible()}
 await page.emulateMedia({forcedColors:'active'});await page.setViewportSize({width:390,height:420});await page.goto('/plan-tasks');await page.evaluate(()=>localStorage.clear());await page.reload();await revealResponsiveWorkspaceOptions(page);const shortOwner=page.getByRole('group',{name:'Visualization'}),shortButtons=shortOwner.getByRole('button'),shortFirst=shortButtons.first(),shortLast=shortButtons.last();await shortFirst.focus();await page.keyboard.press('Shift+Tab');await page.keyboard.press('Tab');for(let index=0;index<(await shortButtons.count())-1;index+=1)await page.keyboard.press('Tab');await expect(shortLast).toBeFocused();await page.waitForTimeout(20);const forcedFocus={...(await focusEvidence(page,shortOwner,shortLast)),forcedColors:await page.evaluate(()=>window.matchMedia('(forced-colors: active)').matches)};await page.keyboard.press('Enter');await revealResponsiveWorkspaceOptions(page);await expect(shortLast).toHaveAttribute('aria-pressed','true');const shortGeometry=await pageGeometry(page);await proof.prove('forced-colors.active',async()=>expect(forcedFocus.forcedColors).toBe(true));await proof.prove('focus.visible',async()=>{expect(forcedFocus.active).toBe(true);expect(forcedFocus.focusVisible).toBe(true);expect(forcedFocus.outlineStyle).not.toBe('none');expect(Number.parseFloat(forcedFocus.outlineWidth)).toBeGreaterThan(0);expect(Number.parseFloat(forcedFocus.outlineOffset)).toBeGreaterThan(0);expect(forcedFocus.fully_inside_owner).toBe(true);expect(forcedFocus.fully_inside_viewport).toBe(true)});await proof.prove('semantics.role-name-state',async()=>{expect(await hiddenResponsiveCount(page)).toBe(0);expect(await shortOwner.count()).toBe(1);expect(await shortButtons.count()).toBeGreaterThanOrEqual(3)})
...
matrixTest('chg153-mobile-navigation-shell',async({page,proof})=>{
 ...
 const checkClosed=async(surface:typeof chg153Surfaces[number],width:390|320)=>{
  const height=width===390?844:800;await page.setViewportSize({width,height});await navigateChg153Surface(page,surface.path);const heading=page.getByRole('heading',{name:surface.heading,exact:true});await expect(heading).toBeVisible();const material=page.locator('.workspace-summary:visible, .command-bar:visible, .workspace-primary > *:visible').first();await expect(material).toBeVisible();const headingBounds=await heading.boundingBox(),materialBounds=await material.boundingBox();const geometry=await pageGeometry(page);const overflowingNodes=await page.evaluate(()=>[...document.body.querySelectorAll('*')].map(element=>{const bounds=element.getBoundingClientRect();return {tag:element.tagName.toLowerCase(),id:element.id,class_name:element.getAttribute('class')??'',text:element.textContent?.trim().slice(0,80)??'',parent_tag:element.parentElement?.tagName.toLowerCase()??'',parent_class:element.parentElement?.getAttribute('class')??'',left:bounds.left,right:bounds.right,top:bounds.top,bottom:bounds.bottom,width:bounds.width,scroll_width:element.scrollWidth,client_width:element.clientWidth,overflow_x:getComputedStyle(element).overflowX}}).filter(element=>element.left<-.5||element.right>innerWidth+.5).sort((left,right)=>right.right-left.right).slice(0,12));const trigger=page.getByRole('button',{name:/Navigation/});await expect(trigger).toBeVisible();await expect(trigger).toHaveAttribute('aria-expanded','false');expect(await navigationVisibleCount(page)).toBe(0);const active=page.locator('#mobile-navigation-panel a[aria-current="page"]');await expect(active).toHaveCount(1);const result={surface_key:surface.key,path:surface.path,heading:surface.heading,width,height,heading_bounds:headingBounds,material_task_start_bounds:materialBounds,heading_intersects_initial_viewport:!!headingBounds&&headingBounds.y<height&&headingBounds.y+headingBounds.height>0,material_intersects_initial_viewport:!!materialBounds&&materialBounds.y<height&&materialBounds.y+materialBounds.height>0,trigger_visible:await trigger.isVisible(),trigger_label:await trigger.textContent(),navigation_expanded:await trigger.getAttribute('aria-expanded'),visible_destination_count:await navigationVisibleCount(page),active_destination:await active.getAttribute('href'),page_geometry:geometry,overflowing_nodes:overflowingNodes,document_width_bounded:geometry.document_scroll_width<=width&&geometry.body_scroll_width<=width};expect(result.heading_intersects_initial_viewport,JSON.stringify(result)).toBe(true);expect(result.material_intersects_initial_viewport,JSON.stringify(result)).toBe(true);expect(result.document_width_bounded,JSON.stringify(result)).toBe(true);initial[`${surface.key}@${width}`]=result;return result
 }
 ...
 await proof.prove('semantics.role-name-state',async()=>{expect(chg153Surfaces.length).toBe(21);await expect(page.locator('#mobile-navigation-panel nav[aria-label="Main navigation"] a')).toHaveCount(21);const configuredPaths=await page.locator('#mobile-navigation-panel nav[aria-label="Main navigation"] a').evaluateAll(links=>links.map(link=>new URL((link as HTMLAnchorElement).href).pathname));expect(configuredPaths).toHaveLength(chg153Surfaces.length);expect(new Set(configuredPaths)).toEqual(new Set(chg153Surfaces.map(surface=>surface.path)))})
