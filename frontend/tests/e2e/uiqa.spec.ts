...
matrixTest('chg153-mobile-navigation-shell',async({page,proof})=>{
 ...
 const screenshot=async(name:string)=>{await page.screenshot({path:resolve(evidenceDir,name),fullPage:true});return `evidence/current/uiqa/rendered/chg153/${name}`}
 ...
 const planningState={};await page.setViewportSize({width:390,height:844});await page.goto('/plan-tasks');const planningMode=page.getByRole('group',{name:'Visualization'}).getByRole('button',{name:'Gantt',exact:true});const planningSearch=page.getByRole('textbox').first();await planningMode.click();await planningSearch.fill('task');const planningFilter=page.locator('.command-bar select').first();let filterValue:string|null=null;if(await planningFilter.count()&&await planningFilter.locator('option').count()>1){await planningFilter.selectOption({index:1});filterValue=await planningFilter.inputValue()}await page.getByLabel('Appearance',{exact:true}).selectOption('light');await page.getByLabel('Theme',{exact:true}).selectOption('clarity');await page.getByLabel('Contrast',{exact:true}).selectOption('high');const stateBefore={visualization:await planningMode.getAttribute('aria-pressed'),search:await planningSearch.inputValue(),filter:filterValue,appearance:await page.getByLabel('Appearance',{exact:true}).inputValue(),theme:await page.getByLabel('Theme',{exact:true}).inputValue(),contrast:await page.getByLabel('Contrast',{exact:true}).inputValue(),tenant:await page.getByLabel('Tenant',{exact:true}).inputValue()};await page.setViewportSize({width:1440,height:900});await page.setViewportSize({width:390,height:844});await expect(planningMode).toHaveAttribute('aria-pressed','true');await expect(planningSearch).toHaveValue('task');if(filterValue!==null)await expect(planningFilter).toHaveValue(filterValue);const stateAfter390={visualization:await planningMode.getAttribute('aria-pressed'),search:await planningSearch.inputValue(),filter:filterValue,appearance:await page.getByLabel('Appearance',{exact:true}).inputValue(),theme:await page.getByLabel('Theme',{exact:true}).inputValue(),contrast:await page.getByLabel('Contrast',{exact:true}).inputValue(),tenant:await page.getByLabel('Tenant',{exact:true}).inputValue()};focusedEvidence.push(await screenshot('planning-mobile-closed-390x844.png'));await page.getByRole('button',{name:/Navigation/}).click();focusedEvidence.push(await screenshot('planning-mobile-open-390x844.png'));await page.getByRole('button',{name:/Navigation/}).click();await page.setViewportSize({width:320,height:800});await expect(page.getByRole('heading',{level:1})).toBeVisible();focusedEvidence.push(await screenshot('planning-mobile-closed-320x800.png'));await page.getByRole('button',{name:/Navigation/}).click();focusedEvidence.push(await screenshot('planning-mobile-open-320x800.png'));planningState.before=stateBefore;planningState.after_390=stateAfter390;planningState.after_320={visualization:await planningMode.getAttribute('aria-pressed'),search:await planningSearch.inputValue(),filter:filterValue,appearance:await page.getByLabel('Appearance',{exact:true}).inputValue(),theme:await page.getByLabel('Theme',{exact:true}).inputValue(),contrast:await page.getByLabel('Contrast',{exact:true}).inputValue(),tenant:await page.getByLabel('Tenant',{exact:true}).inputValue()};await page.getByRole('button',{name:/Navigation/}).click()
