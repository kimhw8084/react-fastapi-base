...
matrixTest('chg153-system-responsive-geometry',async({page,proof})=>{
 ...
 for(const width of [1440,390,320]){
  ...
  await first.click();await expect(first).toHaveAttribute('aria-selected','true');await pointerScrollToEnd(page,owner,last);const lastVisibleByPointer=await boxFullyInside(owner,last);await expect(last).toBeVisible();await last.click();await revealResponsiveWorkspaceOptions(page);await expect(last).toHaveAttribute('aria-selected','true');pointerWorked=pointerWorked||lastVisibleByPointer
