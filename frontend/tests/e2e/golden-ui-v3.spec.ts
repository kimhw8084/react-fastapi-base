...
test('CHG-185 desktop active navigation reveals deep destinations without needless scrolling',async({page})=>{
 ...
 const inspect=async(path:string)=>page.locator(`.sidebar nav a[href="${path}"]`).evaluate(element=>{const item=element.getBoundingClientRect(),scrollport=element.closest('nav')!.getBoundingClientRect();return {label:element.textContent?.trim(),top:item.top,bottom:item.bottom,scroll_top:element.closest('nav')!.scrollTop,inside:item.top>=scrollport.top&&item.bottom<=scrollport.bottom}})
 ...
 await page.goto('/incidents');await expect(page.getByRole('heading',{name:'Incident command',exact:true})).toBeVisible();await expect.poll(async()=>inspect('/incidents'),{message:'Service operations active navigation link must be fully inside the sidebar scrollport after route stabilization'}).toMatchObject({inside:true});const serviceAfter=await inspect('/incidents');expect(serviceAfter.inside).toBe(true);expect(await serviceLink.getAttribute('aria-current')).toBe('page');const serviceShot=await capture(page,'desktop-nav-service-operations-1440x900.png')
