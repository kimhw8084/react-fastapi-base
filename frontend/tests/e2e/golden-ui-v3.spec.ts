...
test('CHG-185 Process Recipes remains usable and bounded at 320 CSS px',async({page})=>{
 ...
 await recipeList.getByRole('button').first().focus()
 ...
 const focusedRecipeBounds=await lastRecipe.boundingBox(),focusedListBounds=await recipeList.boundingBox()
 expect(focusedRecipeBounds&&focusedListBounds&&focusedRecipeBounds.x>=focusedListBounds.x&&focusedRecipeBounds.x+focusedRecipeBounds.width<=focusedListBounds.x+focusedListBounds.width).toBe(true)
