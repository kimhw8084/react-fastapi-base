...
const dashboardModel=await import(pathToFileURL(join(root,'platform/workspace/dashboardModel.js')))
const riskModel=await import(pathToFileURL(join(root,'platform/workspace/riskModel.js')))
...
test('runtime rejects mixed content, URL credentials, paths, scripts and unknown settings',()=>{for(const patch of [{apiBase:'http://api.test'},{apiBase:'https://u:p@api.test'},{apiBase:'https://api.test/api/v1'},{apiBase:'https://api.test/'},{apiBase:'javascript:alert(1)'},{apiBase:'https://api.test/;x'},{secret:'x'},{defaultTheme:'invented'}])assert.throws(()=>parseRuntime({...config,...patch}))})
...
test('dashboard composer sanitizes widgets, persists variables and responds to viewport',()=>{const layout=dashboardModel.sanitizeDashboard({columns:9,variables:{status:'open',secret:123},widgets:[{id:'chart',kind:'chart',title:'Status',span:2,visible:true},{id:'bad',kind:'unknown',title:'Bad'}]});assert.equal(layout.columns,3);assert.deepEqual(layout.variables,{status:'open'});assert.equal(layout.widgets.length,1);assert.equal(dashboardModel.responsiveColumns(layout,500),1);assert.deepEqual(dashboardModel.crossFilter([{status:'open'},{status:'done'}],'status','open',row=>row.status),[{status:'open'}])})
test('risk model keeps scoring server-parity friendly and exposes heatmap/effect bands',()=>{const before={severity:8,occurrence:6,detection:5},after={severity:4,occurrence:3,detection:2,residualSeverity:3,residualOccurrence:2,residualDetection:2};assert.equal(riskModel.rpn(before),240);assert.equal(riskModel.residualRpn(after),12);assert.equal(riskModel.riskBand(400),'critical');assert.equal(riskModel.heatmap([before])[7][5],1);assert.equal(riskModel.mitigationEffect(before,after).improved,true)})
test('workspace state migrates legacy views and exposes deterministic server conflict reconciliation',()=>{const definition={key:'x',label:'X',description:'',schema_version:1,fields:[],columns:[],capabilities:[],primary_field:'title',filter_keys:[],sort_keys:['updated_at'],visualizations:['table']};const local=workspaceState.migrateWorkspaceView(definition,['table'],{sort:'updated_at',direction:'asc'});assert.equal(local.schema_version,2);assert.deepEqual(local.sorts,[{key:'updated_at',direction:'asc'}]);const result=workspaceState.reconcileWorkspaceView(definition,['table'],local,{...local,density:'compact'});assert.equal(result.conflict,true);assert.equal(result.winner,'server');assert.equal(result.view.density,'compact')})
