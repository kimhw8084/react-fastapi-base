...
function GroupedSemanticGrid<T extends BaseRecord>({rows,definition,density,columns,scope,groupBy,onOpen,onPeek,onHover,onContext,onSelection}:Props<T>&{groupBy:string}){
 ...
 return <div className={`grouped-data-grid ${density}`} aria-label={`${definition.label} grouped by ${fieldLabel(definition,groupBy)}`}>
  {groups.map(group=>{const isCollapsed=collapsed.has(group.key);const selectedCount=group.rows.filter(row=>selected.has(row.id)).length;const groupField=definition.fields.find(field=>field.key===groupBy);const groupLabel=groupField?fieldDisplayText(groupField,group.label):group.label;return <section className="grouped-table-section" key={group.key}>
   <header className="grouped-table-header">
    <button className="group-toggle" aria-expanded={!isCollapsed} onClick={()=>setCollapsed(current=>{const next=new Set(current);next.has(group.key)?next.delete(group.key):next.add(group.key);return next})}><Icon className="group-toggle-icon" name="chevron" style={{transform:isCollapsed?'rotate(-90deg)':undefined}}/> <strong>{groupLabel}</strong> <span>{group.rows.length}</span></button>
    <label><input type="checkbox" aria-label={`Select all ${groupLabel}`} checked={selectedCount===group.rows.length&&group.rows.length>0} ref={node=>{if(node)node.indeterminate=selectedCount>0&&selectedCount<group.rows.length}} onChange={event=>toggleGroup(group.rows,event.target.checked)}/> {selectedCount?`${selectedCount} selected`:'Select group'}</label>
   </header>
...
function StandardDataGrid<T extends BaseRecord>({rows,definition,density,columns,scope,onOpen,onPeek,onHover,onContext,onSelection,onColumns}:Props<T>){
  ...
  const defs=useMemo<ColDef<T>[]>(()=>[
    ...definition.columns.map((key):ColDef<T>=>({
      colId:key,headerName:fieldLabel(definition,key),
  ...
  const gridHeight=gridHeightForRows(rows.length,density)
  return <div className="golden-grid" aria-label={`${definition.label} data grid`} style={{height:gridHeight}}>
    <AgGridReact<T> ref={grid} rowData={rows} columnDefs={defs} getRowId={params=>params.data.id}
      ...
      rowHeight={rowHeight} headerHeight={44} suppressContextMenu
