...
export function criticalPath(tasks:readonly PlanningTaskNode[],dependencies:readonly PlanningDependency[]):string[]{
 ...
 for(const edge of dependencies){if(!durations.has(edge.source)||!durations.has(edge.target))continue;(next.get(edge.source)??(next.set(edge.source,[]),next.get(edge.source)!)).push(edge.target)}
