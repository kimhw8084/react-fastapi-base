...
export function FieldValue({field,value}:{field:FieldDefinition;value:unknown}){
 ...
 if(field.kind==='json')return <pre className="field-structured-value"><code>{JSON.stringify(value,null,2)}</code></pre>
 if(field.kind==='multiselect')return <span className="field-chip-list">{(Array.isArray(value)?value:[]).map(item=><span className="field-chip" key={String(item)}>{String(item).replaceAll('_',' ')}</span>)}</span>
 ...
 if(['integer','number','percent','duration','scientific','unit_number'].includes(field.kind))return <span className="tabular-value">{displayNumber(value,field)}</span>
