...
export function TableWorkspace<T extends BaseRecord>({adapter,api,tenant,user,permissions,view,onViewChange,searchInput,onSearchInput,viewTools}:Props<T>){
  ...
  const searchControl=<label className="search-field"><span className="sr-only">Search records</span><span aria-hidden="true"><Icon name="search"/></span><input aria-label="Search records" value={searchInput} maxLength={200} placeholder={`Search ${adapter.definition.label.toLowerCase()}…`} onChange={event=>onSearchInput(event.target.value)}/></label>
  const datasetControl=<div className="segmented" aria-label="Dataset"><button aria-pressed={!view.archived} onClick={()=>{setView({...view,archived:false});setOffset(0)}}>Active</button><button aria-pressed={view.archived} onClick={()=>{setView({...view,archived:true});setOffset(0)}}>Archived</button></div>
  const filters=adapter.definition.filter_keys.map(key=>{const field=adapter.definition.fields.find(value=>value.key===key);return field?<label key={key}>{field.label}<select aria-label={`Filter by ${field.label.toLowerCase()}`} value={view.filters[key]??''} onChange={event=>{const filters={...view.filters};if(event.target.value)filters[key]=event.target.value;else delete filters[key];setView({...view,filters});setOffset(0)}}><option value="">All {field.label.toLowerCase()}</option>{field.choices.map(value=><option value={value} key={value}>{fieldDisplayText(field,value)}</option>)}</select></label>:null})
  const sorting=<><label>Sort all results<select aria-label="Sort all results" value={view.sort} onChange={event=>{setView({...view,sort:event.target.value});setOffset(0)}}>{adapter.definition.sort_keys.map(key=><option key={key} value={key}>{fieldLabel(adapter.definition,key)}</option>)}</select></label><button aria-label="Toggle sort direction" onClick={()=>setView({...view,direction:view.direction==='asc'?'desc':'asc'})}><Icon name={view.direction==='asc'?'sort-ascending':'sort-descending'}/>{view.direction==='asc'?' Ascending':' Descending'}</button></>
  const displayTrigger=<button className="workspace-display-trigger" aria-expanded={Boolean(displayAnchor)} onClick={displayButton}><Icon name="view"/> Display{view.group_by?` · ${fieldLabel(adapter.definition,view.group_by)}`:''}</button>
  const exportImport=<div className="toolbar-actions">{permissions.includes('export')&&adapter.definition.capabilities.includes('csv')&&<button onClick={()=>exportData.mutate()} disabled={exportData.isPending}>Export CSV</button>}{permissions.includes('import')&&adapter.definition.capabilities.includes('csv')&&adapter.renderExchange&&<button onClick={()=>setExchange(true)}>Import CSV</button>}</div>
  const summary=`${appliedWorkspaceViewSummary(adapter.definition,view)}${params.get('view')?' · Saved view applied':''}`
  const toolbar=<ResponsiveWorkspaceControls primary={<>{searchControl}{datasetControl}</>} summary={summary}><>{filters}{sorting}{displayTrigger}<div className="workspace-mobile-display-controls"><TableDisplayControls inline open anchor={{x:0,y:0}} definition={adapter.definition} view={view} onChange={next=>setView(next)} onClose={()=>undefined}/></div>{viewTools}{exportImport}</></ResponsiveWorkspaceControls>
  return <WorkspaceShell eyebrow="Company workspace" title={adapter.definition.label} description={adapter.definition.description} actions={<><button onClick={refresh} aria-label="Refresh workspace"><Icon name="refresh"/> Refresh</button>{canWrite&&<button className="primary" onClick={()=>setForm('new')}><Icon name="add"/> New {adapter.singular}</button>}</>} metrics={[{label:'Matching records',value:records.data?.total??'—'},{label:'Selected on this page',value:selection.length},{label:'Dataset',value:view.archived?'Archived':'Active',className:'summary-word'},{label:'Access',value:canWrite?'Editor':'Read only',className:'summary-word'}]} toolbar={toolbar} notice={notice?<div className="notice" role="status">{notice}<button aria-label="Dismiss notification" onClick={()=>setNotice('')}><Icon name="close"/></button></div>:undefined} footer={<><span>{records.data?`${records.data.total===0?0:offset+1}–${offset+records.data.items.length} of ${records.data.total}`:'No result loaded'} · Search/filter/sort are server-scoped{view.group_by?' · grouping organizes this loaded page':''}.</span><div><button disabled={offset===0||records.isFetching} onClick={()=>setOffset(Math.max(0,offset-50))}>Previous</button><button disabled={!records.data||offset+50>=records.data.total||records.isFetching} onClick={()=>setOffset(offset+50)}>Next</button></div></>}>
    ...
    {records.isPending?<div className="loading-state" role="status">Loading records…</div>:!records.isError&&records.data?.items.length===0?<WorkspaceEmptyState label={adapter.definition.label} singular={adapter.singular} view={view} searchInput={searchInput} canWrite={canWrite} onViewChange={setView} onSearchInput={onSearchInput} onCreate={()=>setForm('new')}/>:!records.isError&&<DataGrid<T> rows={records.data?.items??[]} definition={adapter.definition} density={view.density??'comfortable'} columns={view.columns??[]} groupBy={view.group_by??''} scope={scope} onOpen={openRow} onPeek={setPeek} onHover={handleHover} onContext={(row,anchor)=>{setHover(null);setContext({row,anchor})}} onSelection={setSelection} onColumns={columns=>setView(current=>({...current,columns}))}/>}
