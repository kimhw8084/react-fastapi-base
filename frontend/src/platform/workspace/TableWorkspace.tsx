...
export function TableWorkspace<T extends BaseRecord>({adapter,api,tenant,user,permissions,view,onViewChange,searchInput,onSearchInput,viewTools}:Props<T>){
  ...
  return <WorkspaceShell eyebrow="Company workspace" title={adapter.definition.label} description={adapter.definition.description} actions={<><button onClick={refresh} aria-label="Refresh workspace"><Icon name="refresh"/> Refresh</button>{canWrite&&<button className="primary" onClick={()=>setForm('new')}><Icon name="add"/> New {adapter.singular}</button>}</>} metrics={[{label:'Matching records',value:records.data?.total??'—'},{label:'Selected on this page',value:selection.length},{label:'Dataset',value:view.archived?'Archived':'Active',className:'summary-word'},{label:'Access',value:canWrite?'Editor':'Read only',className:'summary-word'}]} commandBar={<>
      <label className="search-field"><span className="sr-only">Search records</span><span aria-hidden="true"><Icon name="search"/></span><input aria-label="Search records" value={searchInput} maxLength={200} placeholder={`Search ${adapter.definition.label.toLowerCase()}…`} onChange={event=>onSearchInput(event.target.value)}/></label>
      ...
      <button aria-label="Toggle sort direction" onClick={()=>setView({...view,direction:view.direction==='asc'?'desc':'asc'})}><Icon name={view.direction==='asc'?'sort-ascending':'sort-descending'}/>{view.direction==='asc'?' Ascending':' Descending'}</button>
      <button aria-expanded={Boolean(displayAnchor)} onClick={displayButton}><Icon name="view"/> Display{view.group_by?` · ${adapter.definition.fields.find(field=>field.key===view.group_by)?.label??view.group_by}`:''}</button>
    </>} secondaryBar={<>{viewTools}<div className="toolbar-actions">{permissions.includes('export')&&adapter.definition.capabilities.includes('csv')&&<button onClick={()=>exportData.mutate()} disabled={exportData.isPending}>Export CSV</button>}{permissions.includes('import')&&adapter.definition.capabilities.includes('csv')&&adapter.renderExchange&&<button onClick={()=>setExchange(true)}>Import CSV</button>}</div></>} notice={notice?<div className="notice" role="status">{notice}<button aria-label="Dismiss notification" onClick={()=>setNotice('')}><Icon name="close"/></button></div>:undefined} footer={<><span>{records.data?`${records.data.total===0?0:offset+1}–${offset+records.data.items.length} of ${records.data.total}`:'No result loaded'} · Search/filter/sort are server-scoped{view.group_by?' · grouping organizes this loaded page':''}.</span><div><button disabled={offset===0||records.isFetching} onClick={()=>setOffset(Math.max(0,offset-50))}>Previous</button><button disabled={!records.data||offset+50>=records.data.total||records.isFetching} onClick={()=>setOffset(offset+50)}>Next</button></div></>}>
