...
export function FieldInput({field,draft,onChange,invalid,autoFocus=false}:{field:FieldDefinition;draft:Draft;onChange:(draft:Draft)=>void;invalid:boolean;autoFocus?:boolean}){
 ...
 if(field.kind==='select')return <select id={`record-field-${field.key}`} value={value} required={field.required} aria-invalid={invalid} autoFocus={autoFocus} onChange={event=>update(event.target.value)}>{field.required&&<option value="" disabled>Choose…</option>}{field.nullable&&<option value="">— None —</option>}{field.choices.map(choice=><option key={choice} value={choice}>{choice.replaceAll('_',' ')}</option>)}</select>
 ...
 if(field.kind==='boolean')return <select id={`record-field-${field.key}`} value={value||(field.nullable?'':'false')} required={field.required} aria-invalid={invalid} autoFocus={autoFocus} onChange={event=>update(event.target.value)}>{field.nullable&&<option value="">— None —</option>}<option value="false">False</option><option value="true">True</option></select>
 const numeric=['integer','number','decimal','percent','duration','scientific','unit_number','range','tolerance'].includes(field.kind)
 if(numeric){
  const input=<input id={`record-field-${field.key}`} type="number" inputMode={field.kind==='integer'?'numeric':'decimal'} step={field.kind==='integer'?1:field.step??'any'} min={field.minimum??undefined} max={field.maximum??undefined} value={value} required={field.required} aria-invalid={invalid} autoFocus={autoFocus} onChange={event=>update(event.target.value)}/>
 ...
 if(field.kind==='date')return <input id={`record-field-${field.key}`} type="date" value={value} required={field.required} aria-invalid={invalid} autoFocus={autoFocus} onChange={event=>update(event.target.value)}/>
 if(field.kind==='datetime')return <input id={`record-field-${field.key}`} type="datetime-local" value={value} required={field.required} aria-invalid={invalid} autoFocus={autoFocus} onChange={event=>update(event.target.value)}/>
 return <input id={`record-field-${field.key}`} type={field.kind==='email'?'email':field.kind==='url'?'url':'text'} value={value} required={field.required} maxLength={field.max_length??undefined} aria-invalid={invalid} autoFocus={autoFocus} onChange={event=>update(event.target.value)}/>
}
