...
export function ProjectionWorkspaceFrame<T extends BaseRecord>({adapter,api,user,tenant,permissions,view,onViewChange,searchInput,onSearchInput,viewTools,projectionKey,title,description,children,limit=500,extraMetrics}:Props<T>){
  ...
  const projectionLabel=visualizationDisplayLabel(projectionKey)
  const metrics=[{label:'Matching records',value:records.data?.total??'—'},{label:'Visualization',value:projectionLabel,className:'summary-word'},{label:'Dataset',value:view.archived?'Archived':'Active',className:'summary-word'},{label:'Access',value:canWrite?'Editor':'Read only',className:'summary-word'},...(extraMetrics?.(rows)??[])]
  const searchControl=<label className="search-field"><span className="sr-only">Search records</span><span aria-hidden="true"><Icon name="search"/></span><input value={searchInput} maxLength={200} aria-label={`Search ${projectionLabel} records`} placeholder={`Search ${adapter.definition.label.toLowerCase()}…`} onChange={event=>onSearchInput(event.target.value)}/></label>
  const datasetControl=<div className="segmented" aria-label="Dataset"><button aria-pressed={!view.archived} onClick={()=>onViewChange(current=>({...current,archived:false}))}>Active</button><button aria-pressed={view.archived} onClick={()=>onViewChange(current=>({...current,archived:true}))}>Archived</button></div>
  const filters=adapter.definition.filter_keys.map(key=>{const field=adapter.definition.fields.find(value=>value.key===key);return field&&field.choices.length?<label key={key}>{field.label}<select aria-label={`Filter ${projectionLabel} by ${field.label.toLowerCase()}`} value={view.filters[key]??''} onChange={event=>onViewChange(current=>{const filters={...current.filters};if(event.target.value)filters[key]=event.target.value;else delete filters[key];return {...current,filters}})}><option value="">All {field.label.toLowerCase()}</option>{field.choices.map(value=><option key={value} value={value}>{fieldDisplayText(field,value)}</option>)}</select></label>:null})
  const summary=`${appliedWorkspaceViewSummary(adapter.definition,view,projectionLabel)}${params.get('view')?' · Saved view applied':''}`
  const toolbar=<ResponsiveWorkspaceControls primary={<>{searchControl}{datasetControl}</>} summary={summary}><>{filters}{viewTools}</></ResponsiveWorkspaceControls>
  return <WorkspaceShell density={projectionKey==='knowledge'?'compact':'default'} eyebrow="Canonical projection" title={title} description={description} actions={<><button onClick={refresh}><Icon name="refresh"/> Refresh</button>{canWrite&&<button className="primary" onClick={()=>setForm('new')}><Icon name="add"/> New {adapter.singular}</button>}</>} metrics={metrics} toolbar={toolbar}>
    {records.isError&&<ErrorNotice error={records.error} retry={()=>{void records.refetch()}}/>}
    {records.isPending?<div className="loading-state" role="status">Loading {projectionLabel}…</div>:!records.isError&&rows.length===0?<WorkspaceEmptyState label={adapter.definition.label} singular={adapter.singular} view={view} searchInput={searchInput} canWrite={canWrite} onViewChange={onViewChange} onSearchInput={onSearchInput} onCreate={()=>setForm('new')}/>:!records.isError&&children(rows,{openRow,peekRow:setPeek,canWrite,refresh})}
