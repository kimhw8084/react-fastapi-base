...
export function ProjectionWorkspaceFrame<T extends BaseRecord>({adapter,api,user,tenant,permissions,view,onViewChange,searchInput,onSearchInput,viewTools,projectionKey,title,description,children,limit=500,extraMetrics}:Props<T>){
  ...
  return <WorkspaceShell eyebrow="Canonical projection" title={title} description={description} actions={<><button onClick={refresh}><Icon name="refresh"/> Refresh</button>{canWrite&&<button className="primary" onClick={()=>setForm('new')}><Icon name="add"/> New {adapter.singular}</button>}</>} metrics={metrics} toolbar={toolbar}>
