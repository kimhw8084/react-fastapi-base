...
import { FieldValue, fieldDisplayText } from './FieldValue'
import { Icon } from '../ui/Icon'
import { appliedWorkspaceViewSummary, ResponsiveWorkspaceControls } from './ResponsiveWorkspaceControls'
...
export function BoardWorkspace<T extends BaseRecord>({adapter,api,user,tenant,permissions,view,onViewChange,searchInput,onSearchInput,viewTools}:Props<T>){
 ...
 const groupLabel=(value:string)=>groupField?fieldDisplayText(groupField,value):value
 ...
 const searchControl=<label className="search-field"><span className="sr-only">Search records</span><span aria-hidden="true"><Icon name="search"/></span><input value={searchInput} maxLength={200} aria-label="Search board records" placeholder={`Search ${adapter.definition.label.toLowerCase()}…`} onChange={event=>onSearchInput(event.target.value)}/></label>
 const datasetControl=<div className="segmented" aria-label="Dataset"><button aria-pressed={!view.archived} onClick={()=>setView(current=>({...current,archived:false}))}>Active</button><button aria-pressed={view.archived} onClick={()=>setView(current=>({...current,archived:true}))}>Archived</button></div>
 const filters=adapter.definition.filter_keys.map(key=>{const field=adapter.definition.fields.find(value=>value.key===key);return field&&field.choices.length?<label key={key}>{field.label}<select aria-label={`Filter board by ${field.label.toLowerCase()}`} value={view.filters[key]??''} onChange={event=>setView(current=>{const filters={...current.filters};if(event.target.value)filters[key]=event.target.value;else delete filters[key];return {...current,filters}})}><option value="">All {field.label.toLowerCase()}</option>{field.choices.map(value=><option key={value} value={value}>{fieldDisplayText(field,value)}</option>)}</select></label>:null})
 const grouping=groupFields.length>0&&<label>Group cards<select value={groupKey} onChange={event=>setView(current=>({...current,group_by:event.target.value}))}>{groupFields.map(field=><option key={field.key} value={field.key}>{field.label}</option>)}</select></label>
 const sorting=<><label>Sort cards<select value={view.sort} onChange={event=>setView(current=>({...current,sort:event.target.value}))}>{adapter.definition.sort_keys.map(key=><option key={key} value={key}>{adapter.definition.fields.find(field=>field.key===key)?.label??key.replaceAll('_',' ')}</option>)}</select></label><button aria-label="Toggle board sort direction" onClick={()=>setView(current=>({...current,direction:current.direction==='asc'?'desc':'asc'}))}><Icon name={view.direction==='asc'?'sort-ascending':'sort-descending'}/>{view.direction==='asc'?' Ascending':' Descending'}</button></>
 const summary=`${appliedWorkspaceViewSummary(adapter.definition,view,'Board')}${params.get('view')?' · Saved view applied':''}`
 const controls=<ResponsiveWorkspaceControls primary={<>{searchControl}{datasetControl}</>} summary={summary}><>{filters}{grouping}{sorting}{viewTools}</></ResponsiveWorkspaceControls>
 return <WorkspaceShell eyebrow="Canonical projection" title={`${adapter.definition.label} board`} description={`The board and table operate on the same ${adapter.singular} records. Moving a card creates a normal audited update rather than board-specific shadow data.`} actions={<><button onClick={refresh}><Icon name="refresh"/> Refresh</button>{canWrite&&<button className="primary" onClick={()=>setForm('new')}><Icon name="add"/> New {adapter.singular}</button>}</>} metrics={[{label:'Matching records',value:records.data?.total??'—'},{label:'Selected',value:selection.length},{label:'Grouping',value:groupField?.label??'None',className:'summary-word'},{label:'Access',value:canWrite?'Editor':'Read only',className:'summary-word'}]} commandBar={controls} secondaryBar={<><div className="toolbar-actions">{selection.length>0&&canWrite&&(!view.archived||permissions.includes('restore'))&&<button className="danger" onClick={()=>setConfirm(view.archived?'restore':'archive')}>{view.archived?'Restore':'Archive'} selected ({selection.length})</button>}</div></>} notice={notice?<div className="notice" role="status">{notice}<button aria-label="Dismiss notification" onClick={()=>setNotice('')}><Icon name="close"/></button></div>:undefined}>
  {records.isError&&<ErrorNotice error={records.error} retry={()=>{void records.refetch()}}/>}
  {move.isError&&<ErrorNotice error={move.error}/>} 
  {records.isPending?<div className="loading-state" role="status">Loading board…</div>:!records.isError&&rows.length===0?<WorkspaceEmptyState label={adapter.definition.label} singular={adapter.singular} view={view} searchInput={searchInput} canWrite={canWrite} onViewChange={setView} onSearchInput={onSearchInput} onCreate={()=>setForm('new')}/>:<div className={`board-scroll density-${view.density}`} role="region" aria-label={`${adapter.definition.label} board`}><div className="board-columns">{columns.map(column=><section className="board-column" key={column.value} aria-label={`${groupLabel(column.value)} ${adapter.definition.label}`}><header><strong>{groupLabel(column.value)}</strong><span>{column.rows.length}</span></header><div className="board-card-list">{column.rows.map(row=><article className="board-card" key={row.id}><div className="board-card-heading"><label className="board-select"><span className="sr-only">Select {String(fieldValue(row,primary)??row.id)}</span><input type="checkbox" checked={selection.some(item=>item.id===row.id)} onChange={event=>toggle(row,event.target.checked)}/></label><button className="board-card-title" onClick={()=>openRow(row)}>{String(fieldValue(row,primary)??row.id)}</button><button className="peek-button" onClick={()=>setPeek(row)} aria-label={`Quick look ${String(fieldValue(row,primary)??row.id)}`}><Icon name="quick-look"/></button></div>{secondary.map(field=><p key={field.key}><span>{field.label}</span><FieldValue field={field} value={fieldValue(row,field.key)}/></p>)}{canWrite&&!row.archived&&groupField&&<label className="board-move">Move to<select aria-label={`Move ${String(fieldValue(row,primary)??row.id)} to ${groupField.label}`} value={String(fieldValue(row,groupKey)??'')} disabled={move.isPending} onChange={event=>move.mutate({row,value:event.target.value})}>{values.map(value=><option key={value} value={value}>{fieldDisplayText(groupField,value)}</option>)}</select></label>}</article>)}</div></section>)}{ungrouped.length>0&&<section className="board-column"><header><strong>Other</strong><span>{ungrouped.length}</span></header><div className="board-card-list">{ungrouped.map(row=><article className="board-card" key={row.id}><button className="board-card-title" onClick={()=>openRow(row)}>{String(fieldValue(row,primary)??row.id)}</button></article>)}</div></section>}</div></div>}
