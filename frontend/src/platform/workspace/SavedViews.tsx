...
export function SavedViews({ api, workspace, tenant, user, definition, canShare, onApply }: Props) {
  ...
  useEffect(() => {
    const onlineState = () => setOnline(true)
    const offlineState = () => setOnline(false)
    window.addEventListener('online', onlineState)
    window.addEventListener('offline', offlineState)
    return () => { window.removeEventListener('online', onlineState); window.removeEventListener('offline', offlineState) }
  }, [])
  ...
  return <div className="saved-views">
    ...
    {!online && <span className="notice" role="status">Offline: current filters and layout remain local. Saved-view changes will be available when you reconnect.</span>}
    ...
    {open && <Dialog title={selected ? 'Update saved view' : 'Save current view'} onClose={() => setOpen(false)} dirty={name.length > 0} busy={save.isPending} footer={<><button type="button" onClick={() => setOpen(false)} disabled={save.isPending}>Cancel</button><button form="save-view-form" type="submit" className="primary" disabled={save.isPending}>{save.isPending ? 'Saving…' : 'Save view'}</button></>}> 
      <form id="save-view-form" className="form-stack" onSubmit={event => { event.preventDefault(); save.mutate() }}>{save.isError && <ErrorNotice error={save.error}/>} {conflictView && <p className="notice" role="alert">This view changed elsewhere. Reconcile with the server version before saving again. <button type="button" onClick={() => { setSelected(conflictView); setName(conflictView.name); setScope(conflictView.scope); setTeamId(conflictView.team_id ?? ''); setIsFavorite(conflictView.is_favorite); setIsDefault(conflictView.is_default); save.reset() }}>Use server version</button> <button type="button" onClick={() => { setSelected(null); setName(`${name || conflictView.name} (local copy)`.slice(0, 120)); setScope('personal'); setTeamId(''); save.reset() }}>Save local changes as a copy</button></p>}
