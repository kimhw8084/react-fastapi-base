...
export function TableDisplayControls({open,anchor,definition,view,onChange,onClose,inline=false}:{open:boolean;anchor:FloatingAnchor;definition:WorkspaceDefinition;view:ViewDefinition;onChange:(view:ViewDefinition)=>void;onClose:()=>void;inline?:boolean}){
 ...
 const controls=<div className="display-controls-grid">
 ...
 return inline?<section className="display-controls-inline" aria-label="Display controls">{controls}</section>:<FloatingPanelShell open={open} title="Display controls" anchor={anchor} onClose={onClose} width={360} className="display-controls-popover">{controls}</FloatingPanelShell>
}
