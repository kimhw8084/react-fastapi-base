...
export function sanitizeDashboard(value:unknown):DashboardLayout{
 ...
 const variables=source.variables&&typeof source.variables==='object'?Object.fromEntries(Object.entries(source.variables).filter(([key,item])=>['timeRange','status','owner','category'].includes(key)&&typeof item==='string').map(([key,item])=>[key,String(item).slice(0,80)])):{}
 const widgets=Array.isArray(source.widgets)?source.widgets.flatMap(raw=>{if(!raw||typeof raw!=='object')return[];const item=raw as Partial<DashboardWidget>;if(typeof item.id!=='string'||typeof item.title!=='string'||!['metric','chart','table','timeline','health','alert','markdown'].includes(String(item.kind)))return[];const span=item.span===1||item.span===2||item.span===4?item.span:3;return[{id:item.id.slice(0,60),kind:item.kind as DashboardWidgetKind,title:item.title.slice(0,120),span,visible:item.visible!==false,filterKey:typeof item.filterKey==='string'?item.filterKey:undefined}] as DashboardWidget[]}):[]
