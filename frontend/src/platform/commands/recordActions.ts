...
export function createRecordActionRegistry<T extends BaseRecord>(adapter:WorkspaceAdapter<T>,row:T,callbacks:RecordActionCallbacks,permissions:readonly string[],selectionCount=1):ActionRegistry{
 ...
 add('record.open','Open dossier',callbacks.open,['context','keyboard','palette','dossier','inspector'])
 add('record.peek','Quick look',callbacks.peek,['toolbar','context','keyboard','palette','selection','dossier','inspector'],{requiresSelection:true})
 if(callbacks.edit&&canWrite) add('record.edit','Edit record',callbacks.edit,['toolbar','context','keyboard','palette','selection','dossier','inspector'],{permission:'write'})
