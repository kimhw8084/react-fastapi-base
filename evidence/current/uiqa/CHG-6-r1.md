...
| 006 | `REPRODUCED_ROOT_DEFECT` | `experience-lab/src/scheduling.ts` TraceWaterfall used the same page-color label rule; the light info trace bar was `3.43:1`. | The shared Lab semantic label-token correction also owns `.trace-bar`; no feature-local screenshot color was added. After trace minima are `4.95:1` (light normal/high) and `7.98:1` (dark normal/high). Covered by the same Lab browser test and JSON. |
