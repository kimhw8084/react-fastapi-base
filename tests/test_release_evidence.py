...
def test_readiness_matrix_is_deterministic_and_exactly_enumerates_gates():
    ...
    first = RELEASE.build_readiness_matrix(source_commit='a' * 40, source_digest='b' * 64, version='1.0.0-rc.12', results=results)
    second = RELEASE.build_readiness_matrix(source_commit='a' * 40, source_digest='b' * 64, version='1.0.0-rc.12', results=results)
...
def test_local_matrix_keeps_company_gates_unproven_even_when_local_checks_pass():
    ...
    matrix = RELEASE.build_readiness_matrix(source_commit='a' * 40, source_digest='b' * 64, version='1.0.0-rc.12', results=results)
...
def test_build_evidence_does_not_claim_acceptance_or_integration():
    matrix = RELEASE.build_readiness_matrix(source_commit='a' * 40, source_digest='b' * 64, version='1.0.0-rc.12', results=[{'name': 'all-code-checks', 'status': 'PASS'}])
...
def test_written_build_manifest_uses_target_base_and_not_post_acceptance_fields(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(RELEASE, 'UIQA_MATRIX_PATH', uiqa_path)
    ...
    manifest = json.loads((release_dir / 'rc12-manifest.json').read_text())
    binding = json.loads((release_dir / 'rc12-evidence-binding.json').read_text())
