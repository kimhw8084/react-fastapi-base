...
def attach(session,actor: Actor,workspace: str,entity_id: str,data: AttachmentUpload) -> AttachmentRead:
    actor.require('write')
    name=data.filename
    ...
    row=Attachment(id=str(uuid4()),workspace=workspace,entity_id=entity_id,filename=name,
        content_type=data.content_type,size=len(content),sha256=hashlib.sha256(content).hexdigest(),
        content=content,created_by=actor.user_id)
