...
class PlanTask(TenantBase):
    ...
    resource_group: Mapped[str | None]=mapped_column(String(120),nullable=True,default=None,index=True)
    effort_hours: Mapped[float]=mapped_column(Float,nullable=False,default=8.0,index=True)
    capacity_hours: Mapped[float]=mapped_column(Float,nullable=False,default=8.0,index=True)
