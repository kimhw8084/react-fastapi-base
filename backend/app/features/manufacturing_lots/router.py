...
from . import service
from .schemas import ManufacturingLotCreate,ManufacturingLotUpdate,ManufacturingLotRead,ManufacturingLotPage,ManufacturingLotBulkRequest,RevertRequest
router=APIRouter(prefix='/manufacturing-lots',tags=['Manufacturing lots']);A=Annotated[Actor,Depends(actor_for)]
@router.get('',response_model=ManufacturingLotPage)
...
@router.post('',response_model=ManufacturingLotRead,status_code=201)
def create(request:Request,actor:A,data:ManufacturingLotCreate,idempotency_key:str|None=Header(None)):
    with write_transaction(request.app.state.database,actor.tenant_id) as db:return execute_once(db,actor,idempotency_key,'manufacturing_lots.create',data.model_dump(),lambda:service.CRUD.create(db,actor,data).model_dump(mode='json'))
@router.post('/bulk',response_model=list[ManufacturingLotRead])
def bulk(request:Request,actor:A,data:ManufacturingLotBulkRequest,idempotency_key:str=Header(...)):
    with write_transaction(request.app.state.database,actor.tenant_id) as db:
        result=execute_once(db,actor,idempotency_key,'manufacturing_lots.bulk',data.model_dump(),lambda:{'items':[row.model_dump(mode='json') for row in service.bulk(db,actor,data)]});return result['items']
@router.get('/{record_id}',response_model=ManufacturingLotRead)
def get_record(request:Request,actor:A,record_id:str):
    actor.require('read')
    with request.app.state.database.session(actor.tenant_id) as db:return ManufacturingLotRead.model_validate(service.CRUD.require(db,record_id))
@router.put('/{record_id}',response_model=ManufacturingLotRead)
def update_record(request:Request,actor:A,record_id:str,data:ManufacturingLotUpdate):
...
@router.post('/{record_id}/lifecycle/{action}',response_model=ManufacturingLotRead)
...
@router.post('/{record_id}/revert',response_model=ManufacturingLotRead)
