...
def _operations_facts(deployment_id: str, source_commit: str, source_digest: str) -> OperationsFacts:
...
def _final_qualification(root: Path, *, deployment_id: str = 'qualification-fixture', identity: RepositoryReleaseIdentity | None = None) -> CompanyQualification:
    ...
    return CompanyQualification(
        ...
        gates=[
            ...
            CompanyQualificationGate(id='operations', status='PASS', evidence=[evidence('operations_report', 'operations')], facts=_operations_facts(deployment_id, source_commit, source_digest)),
...
def test_production_rejects_release_fact_mismatch_against_repository_anchor(tmp_path, monkeypatch, gate_id, field_name):
    ...
    errors = settings.production_errors(scanner_is_noop=False)
    assert any(gate_id.replace('_', ' ') in error.casefold() or 'repository release identity' in error for error in errors)
...
def test_production_missing_or_unreadable_repository_identity_fails_closed(tmp_path, monkeypatch):
    ...
    assert any('repository release identity' in error.casefold() for error in settings.production_errors(scanner_is_noop=False))
    identity_path.mkdir()
    assert any('repository release identity' in error.casefold() for error in settings.production_errors(scanner_is_noop=False))
