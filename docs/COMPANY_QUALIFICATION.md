...
Production also loads the current-version-derived repository-owned release identity, currently `deploy/rc16-release-identity.json`. RC.15 and earlier identities remain explicit historical compatibility artifacts.
...
The app's preflight checks configuration, exact project/profile/version/source/digest/deployment/root binding and evidence shape. It does not substitute for build/test/security scans, a complete platform scope, or a company release authorization. Local/Fabric verification publishes the current-version-derived `evidence/current/release/rc16-readiness-matrix.json`, which is repository evidence and remains `NOT_CERTIFIED`; it does not become an operator qualification file. `evidence/current/storage/qualification.json` is a repository storage-contract result only; final company storage remains `BLOCKED_EXTERNAL`. `evidence/current/reuse/qualification.json` is a repository reusable-platform result only and cannot pass company identity, storage, deployment, operations or overall CompanyQualification.
