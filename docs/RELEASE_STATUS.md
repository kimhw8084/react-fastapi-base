# Release status — react-fastapi-base 1.0.0-rc.16

## Decision

**V1 LOCAL CODE COMPLETE — COMPANY QUALIFICATION PENDING.** RC.16 adds the explicit company storage persistence contract, profile-owned operator maintenance, bounded storage diagnostics and source-bound repository storage qualification on top of RC.15's reusable-platform and release/evidence lifecycle. All locally achievable V1 contracts in [v1-completion-status.json](v1-completion-status.json) remain source-backed verification; this is not a company production certificate.
...
RC.12–RC.15 are immutable integrated historical evidence for their exact sources. RC.16 binds the current release lifecycle artifacts to the verified RC.16 source; repository storage qualification, reusable-platform PASS and local performance/browser gates do not prove the final company/profile identity, storage, deployment, operations, `ui_accessibility` or `performance` qualification.
...
The deterministic RC.16 repository readiness report is `evidence/current/release/rc16-readiness-matrix.json`; its companion manifest and binding are `rc16-manifest.json` and `rc16-evidence-binding.json`. RC.15 and earlier corresponding artifacts remain immutable historical evidence. The source-bound storage qualification is `evidence/current/storage/qualification.json`; the reusable-platform aggregate is `evidence/current/reuse/qualification.json`. Canonical performance evidence is under `evidence/current/performance/`, including `qualification.json` and `browser.json`; all are repository evidence, not an operator-approved qualification file.
