> **Implementation addendum (0.2.0-lab):** see decisions/0001-native-widget-layer.md, EXPERIENCE_LAB.md and COMPONENT_COVERAGE.md. The standalone typed widget Lab is implemented; React integration, full inventory and release certification remain open. This design is not itself production evidence.

# React-FastAPI Base Platform — master design and implementation contract

**Version: 0.1.0-review · Reference: SysGrid 66244b997a70b85e6e887870c96db958f3f0d22d · Release: NOT_CERTIFIED**
...
CSV is an explicit versioned reference format, not a raw database dump. Cells that could become spreadsheet formulas are escaped reversibly, and import restores escaping only for this versioned contract. Preview validates every row. Commit requires the preview fingerprint and revalidates the payload; all rows are inserted atomically. It does not overwrite matching records or restore IDs/history. Imports are capped at 100 rows, exports at 1,000 filtered records. XLSX and multi-entity relationship snapshots remain unfinished.
...
Current runtime candidates: Python 3.13 with exact locally installed backend dependency snapshot; Node 22.12+ in the 22 line; React 18.3.1; Vite 8.2.2; React Router 7.18.3; TanStack Query 5.102.8; AG Grid Community 36.1.0. Frontend versions are candidates: no resolved lock or React build was possible in this session. Do not advertise this combination as validated. The company Node/Python runtime may require a different independently tested compatibility branch.
