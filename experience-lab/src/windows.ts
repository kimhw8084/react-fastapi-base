...
export class WindowGallery extends EngineeringElement<{lastAction:string}> {
 private opener:HTMLElement|null=null;private dirty=false;private step=1;private draftName='';private draftLayout='Table + inspector';private windowCounter=0;
 ...
 override render():void{
  ...
  this.on('[data-popover-action]','click',e=>{this.value.lastAction=(e.currentTarget as HTMLElement).dataset.popoverAction!+' requested in the example.';this.render();});
 }
 private open(kind:WindowKind):void{
  this.dirty=false;this.step=1;this.draftName='';this.draftLayout='Table + inspector';const dialog=document.createElement('dialog');dialog.className=`lab-dialog placement-${kind}`;
  const titleId=`rf-window-title-${++this.windowCounter}-${localId()}`;dialog.setAttribute('aria-labelledby',titleId);
  const render=()=>{
   ...
   if(kind==='wizard'){const name=dialog.querySelector<HTMLInputElement>('input[data-draft]');if(name)name.value=this.draftName;const layout=dialog.querySelector('select');if(layout){layout.value=this.draftLayout;layout.addEventListener('change',()=>{this.draftLayout=layout.value;this.dirty=true;});}}
   if(this.readonlyMode)dialog.querySelectorAll<HTMLInputElement|HTMLTextAreaElement|HTMLSelectElement>('input,textarea,select').forEach(el=>el.disabled=true);
   dialog.querySelectorAll('[data-draft]').forEach(el=>el.addEventListener('input',()=>{this.dirty=true;if(kind==='wizard'&&el instanceof HTMLInputElement)this.draftName=el.value;}));
   ...
   dialog.querySelectorAll('[data-finish]').forEach(el=>el.addEventListener('click',()=>{
    ...
    if(kind==='wizard'&&this.step<3){const draft=dialog.querySelector<HTMLInputElement>('input[data-draft]');if(draft&&draft.value.trim().length<2){dialog.querySelector('.dialog-validation')!.textContent='Enter at least two characters.';draft.focus();return;}this.step++;this.dirty=true;render();return;}
    if(kind==='dialog'||kind==='fullscreen'){const draft=dialog.querySelector<HTMLInputElement>('input[data-draft]');if(!draft?.value.trim()){dialog.querySelector('.dialog-validation')!.textContent='Workspace name is required.';draft?.focus();return;}}
    this.dirty=false;this.value.lastAction=`${title}: local example completed.`;dialog.close();this.render();
   }));
   queueMicrotask(()=>dialog.querySelector<HTMLElement>('input,select,[data-close]')?.focus());
  };
  const requestClose=()=>{
   if(!this.dirty){dialog.close();return;}
   if(dialog.querySelector('[data-discard]'))return;
   const guard=document.createElement('div');guard.className='discard-guard';guard.setAttribute('role','alert');guard.innerHTML='<strong>Discard unsaved changes?</strong><p>Your local edits will be lost.</p><button class="button" data-keep>Keep editing</button><button class="button danger" data-discard>Discard changes</button>';
   dialog.querySelector('.dialog-body')?.append(guard);guard.querySelector('[data-keep]')?.addEventListener('click',()=>{guard.remove();dialog.querySelector<HTMLElement>('[data-draft]')?.focus();});guard.querySelector('[data-discard]')?.addEventListener('click',()=>{this.dirty=false;dialog.close();});guard.querySelector<HTMLElement>('[data-keep]')?.focus();
  };
  dialog.addEventListener('cancel',e=>{e.preventDefault();requestClose();});
  dialog.addEventListener('click',e=>{if(e.target===dialog){const r=dialog.getBoundingClientRect();if((e as MouseEvent).clientX<r.left||(e as MouseEvent).clientX>r.right||(e as MouseEvent).clientY<r.top||(e as MouseEvent).clientY>r.bottom)requestClose();}});
  dialog.addEventListener('close',()=>{dialog.remove();if(this.opener?.isConnected)this.opener.focus();else this.querySelector<HTMLElement>(`[data-window="${kind}"]`)?.focus();});
  render();containDialogFocus(dialog);document.body.append(dialog);dialog.showModal();
 }
}
