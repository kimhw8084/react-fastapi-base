...
import {escapeHtml as h,paginate,recordsCsv,downloadText,type RecordRow} from './model.js';
export class RecordTable extends EngineeringElement<RecordRow[]> {
 private search='';private status='All statuses';private sort:keyof RecordRow='id';private descending=false;private page=1;private grouped=false;private selected=new Set<string>();
 ...
 override render():void{
  ...
  this.on('[data-focus="table-search"]','input',e=>{this.search=(e.target as HTMLInputElement).value;this.page=1;this.selected.clear();this.render();});
  this.on('[data-filter]','change',e=>{this.status=(e.target as HTMLSelectElement).value;this.page=1;this.selected.clear();this.render();});
  this.on('[data-group]','click',()=>{this.grouped=!this.grouped;this.render();});
  this.on('[data-sort]','click',e=>{const key=(e.currentTarget as HTMLElement).dataset.sort as keyof RecordRow;this.descending=this.sort===key?!this.descending:false;this.sort=key;this.render();});
  this.on('[data-select]','change',e=>{const input=e.currentTarget as HTMLInputElement;input.checked?this.selected.add(input.dataset.select!):this.selected.delete(input.dataset.select!);this.render();});
  this.on('[data-all]','change',e=>{if((e.target as HTMLInputElement).checked)rows.forEach(r=>this.selected.add(r.id));else this.selected.clear();this.render();});
  this.on('[data-prev]','click',()=>{this.page--;this.selected.clear();this.render();});this.on('[data-next]','click',()=>{this.page++;this.selected.clear();this.render();});
  ...
  this.on('[data-open]','click',e=>this.select(this.value.find(r=>r.id===(e.currentTarget as HTMLElement).dataset.open)));
  this.on('[data-export]','click',()=>downloadText('work-items.csv',recordsCsv(sorted),'text/csv'));
 }
}
export class WorkBoard extends EngineeringElement<RecordRow[]> {
 ...
 override render():void{
  ...
  this.on('[data-card]','dragstart',e=>{if(this.readonlyMode){e.preventDefault();return;}this.dragId=(e.currentTarget as HTMLElement).dataset.card!;(e as DragEvent).dataTransfer?.setData('text/plain',this.dragId);});
  this.on('[data-card]','dragend',()=>{this.dragId=null;});
  this.on('[data-lane]','dragover',e=>{if(!this.readonlyMode)e.preventDefault();});
  this.on('[data-lane]','drop',e=>{e.preventDefault();if(this.dragId){this.move(this.dragId,(e.currentTarget as HTMLElement).dataset.lane!);this.dragId=null;}});
  this.on('[data-move]','change',e=>{const el=e.target as HTMLSelectElement;this.move(el.dataset.move!,el.value);});
  this.on('[data-detail]','click',e=>this.select(this.value.find(r=>r.id===(e.currentTarget as HTMLElement).dataset.detail)));
 }
 private move(id:string,status:string):void{this.change(this.value.map(r=>r.id===id?{...r,status}:r),'move-status');}
}
register('rf-record-table',RecordTable);register('rf-work-board',WorkBoard);
