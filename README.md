...
`1.0.0-rc.18` — source-bound operational-reliability and failure-mode qualification candidate; V1 local code complete; company qualification pending.
...
Current machine-readable reports are written under `evidence/current/`. They distinguish passing local checks from external company qualification and do not issue a production certificate. The canonical operational-reliability report is `evidence/current/operations/qualification.json`; it exercises deterministic repository failure modes and leaves authentic company operations `BLOCKED_EXTERNAL`. The clean-clone proof validates the exact candidate checkout on its supported runner; macOS-specific qualification is reported only on Darwin and remains distinct from company deployment qualification.
